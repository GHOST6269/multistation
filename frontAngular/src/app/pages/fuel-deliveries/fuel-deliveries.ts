...
export class FuelDeliveries implements OnInit {
  ...
  statusLabel(status: string): string { const labels: Record<string, string> = { DRAFT: 'Brouillon', PENDING: 'En attente', OPEN: 'Ouverte', UNPAID: 'Non payée', PARTIAL: 'Partiellement payée', PARTIALLY_PAID: 'Partiellement payée', PAID: 'Payée', CANCELED: 'Annulée', CANCELLED: 'Annulée' }; return labels[String(status || '').toUpperCase()] ?? status; }
  statusClass(status: string): string { const normalized = String(status || '').toUpperCase(); return normalized === 'PAID' || normalized === 'PAYÉE' ? 'paid' : normalized === 'PARTIAL' || normalized === 'PARTIALLY_PAID' || normalized === 'PARTIELLEMENT PAYÉE' ? 'partial' : 'unpaid'; }
